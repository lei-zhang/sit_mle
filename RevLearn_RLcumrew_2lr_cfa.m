function [nll,d,H,model] = RevLearn_RLcumrew_2lr_cfa(param,data,pps)
%
...
if strcmp(pps, 'mle')
    ...
    cfa  = param(3); % cfa
    disc = param(4); % discount factor, gamma (forgetting rate for reward of others)
    cra  = param(5); % cr against
    crw  = param(6); % cr with
...
if nargout > 3
  ...
  model.pnames = {'lr1','lr2','cfa','disc','cra','crw','temp'};
  model.param  = param;
  model.data   = data;
  model.v      = v;
  model.pe     = pe;
