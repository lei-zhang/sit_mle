function [nll,d,H,model] = RevLearn_RLcumrew_cfa(param,data,pps)
%
...
if strcmp(pps, 'mle')
    ...
    cfa  = param(2); % countfactual attention (
...
elseif strcmp(pps, 'jags')
    lr   = param(1); % learning rate
    cfa  = param(6); % countfactual attention (
    disc = param(5); % discount factor, gamma (forgetting rate for reward of others)
    cra  = param(3); % cr against
    crw  = param(4); % cr with
...
if nargout > 3
  ...
  model.pnames = {'lr','cfa','disc','cra','crw','temp'};
  model.param  = param;
  model.data   = data;
  model.v      = v;
  model.pe     = pe;
