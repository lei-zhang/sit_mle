function [nll,d,H,model] = RevLearn_RLcoh(param,data, pps, flag4)
%
...
if strcmp(pps, 'mle')
    ...
    coha = param(2); % coherence weight for majority against choice1
    cohw = param(3); % coherence weight for majority with choice1
...
v     = zeros(nTrials+1,2);
tmpv  = zeros(nTrials,2);
pe    = zeros(nTrials,1);
penc  = zeros(nTrials,1);
prob1 = zeros(nTrials,2);
prob2 = zeros(nTrials,2);
lik   = zeros(nTrials,1);
...
for t = 1:nTrials
    ...
    %g2 = data(t,10:13);
%     coh_against = length(find(g1~=c1));
%     coh_with    = length(find(g1==c1));
...
%     v(t,c1)   = v(t,c1)   + cohw * coh_with;    % or cohw * (coh_with-1)
    ...
    tmpv(t,3-c1) = v(t,3-c1) + coha * coh_against; % or coha * (coh_against-1)
    tmpv(t,c1)   = v(t,c1)   + cohw * coh_with;    % or cohw * (coh_with-1)
    
    % Do I need to normalize the values here?
    
    % action selection 2
    prob2(t,1)= 1 / (1 + exp(tau*(tmpv(t,2)-tmpv(t,1))));
    prob2(t,2)= 1 / (1 + exp(tau*(tmpv(t,1)-tmpv(t,2))));
...
if nargout > 3
  ...
  model.pnames = {'lr','coh_against','coh_with','temp'};
  model.param  = param;
  model.data   = data;
  model.v      = v;
  model.tmpv   = tmpv;
